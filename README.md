...
```bash
# x86_64
curl -L -o logview.tar.gz https://gitee.com/Mtok/logview/releases/download/v0.17.4/logview_linux_amd64.tar.gz
tar xzf logview.tar.gz && rm logview.tar.gz
sudo mv logview /usr/local/bin/

# ARM64
curl -L -o logview.tar.gz https://gitee.com/Mtok/logview/releases/download/v0.17.4/logview_linux_arm64.tar.gz
tar xzf logview.tar.gz && rm logview.tar.gz
sudo mv logview /usr/local/bin/
...
| `FRP` | 连接列表 → 远程目录 | 已存 frp(stcp) 连接一键直达（参数持久化），或表单新建；隧道自动建立，其余同 SSH 目录浏览；`C-x` 删除光标处已存连接 |
...
**SSH 密码认证**：需要密码的主机连接失败时自动弹出密码输入框（掩码显示），Enter 自动重连；密码仅存内存（会话内同主机免重输，不落盘）。目录浏览和 tail 均支持。SSH 主机层 `C-l` 在主机输入与路径输入间切换焦点。
...
| `+N` / `-N` | 过滤/搜索激活时,触发行旁混入原始流上下文(`+` 往后 / `-` 往前,N 空=5;暗色+┆ 标记插入行) |
| `--` / `++` / `Esc` | 关闭上下文混入(光标回最近过滤行) |
| `o` / `q` | 打开源选择器（K8s/本地/SSH/FRP） |
| `\` | 收起 / 展开底部快捷键提示栏 |
| `S-c` | 清空屏幕 |
| `C-c` | 退出 logview |

**上下文混入态为粘性且可多锚累积**：移动/滚动/详情/复制/书签/跳转/`z` 锚定照常可用，光标可停在插入的暗色上下文行上操作（`e` 折叠与 `v`/`V` 可视选择禁用）；在不同行分别按 `+N`/`-N` 各行的上下文互不清除（同行重复按键以最后一次为准），状态栏常显 `混入 ×K(Esc/-- 关闭)`（K=锚数）。修改搜索词、level 过滤、隐藏词、清屏、切换源时自动退出；follow 新行不进混入视图。
...
|------|------|
| `↑` / `↓`（`C-k` / `C-j`） | 切换设置项（范围 / 格式） |
| `←` / `→`（`h` / `l`） | 调整当前项（范围：当前筛选 / 全部缓冲；格式：原始 / JSON） |
| `Enter` | 确认导出，`Esc` / `q` 取消 |
...
不写日期默认**今天**；全部按绝对时刻比较（跨天不歧义）；非法值红字报错，不静默降级。搜索弹窗内 `C-t` 打开时间快捷片（最近5分钟/今天/昨天等一键插入）。

```
time:>09:00                      今天 09:00 之后（9:00 前导零宽容）
time:>=15:00                     含边界（cron 整点触发行不丢）
time:<10:00:30                   秒级精度
time:>-10m                       相对此刻 10 分钟前（-2d/-1h30m 复合时长）
time:-1h                         简写（负号开头默认 >）
time:>2026-08-26                 指定日期 00:00 之后
time:>2026-08-26T09:00:30        完整时间戳（RFC3339 风格，_ 为别名）
time:10:00..11:00                闭区间（含两端）
time:10:00..11:00 OR time:13:00..14:00    多时段
level:ERROR AND time:>-30s       与其它条件自由组合
```

> 注意：无时区标记的时间戳（绝大多数日志）一律按**本机时区**解释——你在日志里看到什么时间，就用什么时间搜。若日志来自 UTC 时区的容器而本机是 +8，窗口会差 8 小时（可临时 `TZ=UTC logview ...` 对齐）。无时间字段的行不参与任何 time 条件（`NOT time:` 也不会命中它们）；无日期时间戳（`HH:mm:ss.SSS` 日志）按当日时分窗口比较。

字段：`message` `source` `traceId` `thread` `logger`（包含匹配）、`level`（精确匹配）、`time`（时间范围）。

搜索后搜索栏显示匹配位置和总数（如 `[3/15匹配]`）与实际生效的时间窗口（如 `[时间: 08-26 10:00:00~11:00:00]`）。`n` / `N` 跳转匹配结果，对高亮关键词同样有效。
